...
export default class DoopParser extends EventEmitter {
	...
	* @param {Function} [options.wrap] Function to return the final output of each import line, defaults to a simple `import 'X';`. Called as `({block, path})`
	* @returns {String} Generated source code pointing to each extracted block as a relative path
	*/
	index(options) {
		let settings = {
			wrap: ({path}) => `import '${path}';`,
			url: false,
			...options,
		};
