...
export let defaults = {
	...
	index: {
		...
		wrap({block, path, settings}) {
			if (block.attrs.on) {
				let eventBindingSegments = block.attrs.on.split(/\s*,\s*/).map(b => `'${b}'`);
				let eventBinding = eventBindingSegments.length == 1
					? eventBindingSegments[0]
					: '[' + eventBindingSegments + ']';

				return `${settings.globalEmitter}.on(${eventBinding}, ()=> import('${path}'));`;
			} else {
	...
	parse: {
		orphans: false,
		aliases: {
			'endpoint': { attrs: { on: 'endpoint', hotswap: true }},
			'middleware': { attrs: { on: 'middleware', hotswap: true }},
			'schema': { attrs: { on: 'schema' }},
			'util': { attrs: { on: 'util' }},
		},
		getId(block) {
			if (block.attrs.id) { // Use stated ID
				...
				let tag = block.attrs.on && /^[a-z0-9_]+$/i.test(block.attrs.on) ? block.attrs.on // Looks like a valid single variable `on` hook
					: block.tag;
		...
		return new Promise((resolve, reject) =>
			createReadStream(this.path)
				.on('data', data => {
					...
					let dataLines = data.toString().split(/\r?\n/);
